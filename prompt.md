...
|-------------|-----------|
| **Explicit extraction of new task fields** (Research/Investigation, Strict Rules, etc.) in Step 1 | Ensures the workflow actively uses the enriched task information. |
| **Step 2: Check Research/Investigation Field** | Pre‑loads investigation items so Step 3 can answer them directly. |
| **Step 3: Answer investigation items explicitly** | Closes the loop between task‑specified research questions and the workflow’s research phase. |
| **Step 4: Compliance check with Strict Rules & Patterns** | Prevents accidental violation of task‑specific constraints during planning. |
| **Step 7: Adherence to Strict Rules and Anti‑Patterns during implementation** | Reinforces that these are not just planning‑time considerations. |
| **Step 8: Mandatory execution of `Validation Steps`** | Makes validation objective and repeatable, not left to guesswork. |
| **Step 8: Verification of `Testing Requirements`** | Ensures the task’s explicit testing expectations are met. |
| **Step 9: Inclusion of limitations in completion note** | Provides transparency when full validation wasn’t possible. |
| **Source‑of‑Truth Order includes task‑specific rules** | Elevates the importance of the task’s own constraints. |
| **Error Handling references Research/Investigation field** | Suggests a quick check of pre‑gathered information when stuck. |